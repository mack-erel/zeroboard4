...
for ($i = 0; $i < $_zb_c; $i++) {
	if (eregi($_zb_file_list[$i], $_SERVER["PHP_SELF"])) {
		$_zboardis = TRUE;
		break;
...
if ($_zboardis) {
	...
	if (!eregi("write.php", $_SERVER["PHP_SELF"])) {
		...
		if (!$setup["use_showreply"]) if (!$s_que) $s_que = " arrangenum=0 ";
		else $s_que .= " and arrangenum=0 ";

		// 카테고리 : 카테고리가 있을때 category를 검색 조건에 넣음
		if ($category) if (!$s_que) $s_que = " category='$category' ";
		else $s_que .= " and category='$category'";
		...
		if (!isblank($keyword)) {
			...
			if ($ss == "on" && $t_s_que) $t_s_que .= " or subject like '%$keyword%' ";
			elseif ($ss == "on") $t_s_que .= " subject like '%$keyword%' ";
			if ($sc == "on" && $t_s_que) $t_s_que .= " or memo like '%$keyword%' ";
			elseif ($sc == "on") $t_s_que .= " memo like '%$keyword%' ";
	...
	// 모두삭제 버튼
	if ($is_admin) $a_delete_all = "<a onfocus=blur() href='javascript:delete_all()'>";
	else $a_delete_all = "<Zeroboard ";

	// 통계버튼
	if ($setup["use_status"]) $a_status = "<a onfocus=blur() href=javascript:void(window.open('stat.php?id=$id','status','width=400,height=400,statusbar=no,toolbar=no,resizable=no'))>";
	else $a_status = "<Zeroboard ";
	$a_status = "<Zeroboard ";

	// Setup 버튼
	if ($is_admin) $a_setup = "<a onfocus=blur() href='admin_setup.php?exec=view_board&no=$setup[no]&group_no=$setup[group_no]&exec2=modify' target=_blank>";
	else $a_setup = "<Zeroboard ";
