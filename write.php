...
if (!$setup["use_homelink"]) {
	$hide_sitelink1_start = "<!--";
	$hide_sitelink1_end = "-->";
}
if (!$setup["use_filelink"]) {
...
// 이미지 창고 버튼
if ($member["no"] && $setup["grant_imagebox"] >= $member["level"]) $a_imagebox = "<a onfocus=blur() href='javascript:showImageBox(\"$id\")'>";
else $a_imagebox = "<Zeroboard ";
