...
if ($data["is_secret"] && !$is_admin && $data["ismember"] != $member["no"] && $member["level"] > $setup["grant_view_secret"]) {
	if ($member["no"]) {
		...
		if (!$secret_check[0]) {
			...
			include_once $dir . "/ask_password.php";
			foot();
			exit();
		} else {
...
if ($data["homepage"]) $a_homepage = "<a onfocus=blur() href='$data[homepage]' target=_blank>";
else $a_homepage = "<Zetx"; // 홈페이지 주소 링크
...
// 글쓰기버튼
if ($is_admin || $member["level"] <= $setup["grant_write"]) $a_write = "<a onfocus=blur() href='write.php?$href$sort&no=$no&mode=write&sn1=$sn1'>";
else $a_write = "<Zeroboard ";

// 답글 버튼
if (($is_admin || $member["level"] <= $setup["grant_reply"]) && $no && $data["headnum"] > -2000000000) $a_reply = "<a onfocus=blur() href='write.php?$href$sort&no=$no&mode=reply&sn1=$sn1'>";
else $a_reply = "<Zeroboard ";

// 목록 버튼
if ($is_admin || $member["level"] <= $setup["grant_list"]) $a_list = "<a onfocus=blur() href='zboard.php?id=$id&page=$page&page_num=$page_num&category=$category&sn=$sn&ss=$ss&sc=$sc&keyword=$keyword&prev_no=$no&sn1=$sn1&divpage=$divpage&select_arrange=$select_arrange&desc=$desc'>";
else $a_list = "<Zeroboard  ";

// 취소버튼
$a_cancel = "<a onfocus=blur() href='{$_SERVER["PHP_SELF"]}?id=$id'>";

// 삭제버튼
if (($is_admin || $member["level"] <= $setup["grant_delete"] || $data["ismember"] == $member["no"] || !$data["ismember"]) && !$data["child"]) $a_delete = "<a onfocus=blur() href='delete.php?$href$sort&no=$no'>";
else $a_delete = "<Zeroboard ";

// 수정버튼
if (($is_admin || $member["level"] <= $setup["grant_delete"] || $data["ismember"] == $member["no"] || !$data["ismember"]) && $no) $a_modify = "<a onfocus=blur() href='write.php?$href$sort&no=$no&mode=modify'>";
else $a_modify = "<Zeroboard ";

// 파일링크
if ($file_name1) $a_download1 = "<a onfocus=blur() href='download.php?$href$sort&no=$no&file=1'>";
else $a_download1 = "<Zeroboard ";
if ($file_name2) $a_download2 = "<a onfocus=blur() href='download.php?$href$sort&no=$no&file=2'>";
else $a_download2 = "<Zeroboard ";
...
if ($setup["use_comment"]) {
	while ($c_data = mysql_fetch_array($view_comment_result)) {
		...
		if ($is_admin) $show_ip = " title='$c_data[ip]' ";
		else $show_ip = "";
		...
		$_skinTimeStart = getmicrotime();
		include_once $dir . "/view_comment.php";
		$_skinTime += getmicrotime() - $_skinTimeStart;
		flush();
	}
...
// 위, 아래글 출력, 코멘트, 버튼 출력
$_skinTimeStart = getmicrotime();
include_once $dir . "/view_foot.php";
$_skinTime += getmicrotime() - $_skinTimeStart;
...
if ($check_ref[0] > 1) {
	...
	while ($data = mysql_fetch_array($view_result)) {
		...
		if ($data["no"] == $no) $number = "<img src=$dir/arrow.gif border=0>";
		else $number = "&nbsp;";

		// 목록을 출력하는 부분
		$_skinTimeStart = getmicrotime();
		include_once $dir . "/view_list_main.php";
		$_skinTime += getmicrotime() - $_skinTimeStart;
	}
