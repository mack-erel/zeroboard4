...
// 글쓰기버튼
if ($is_admin || $member["level"] <= $setup["grant_write"]) $a_write = "<a onfocus=blur() href='write.php?$href$sort&no=$no&mode=write&sn1=$sn1&divpage=$divpage'>";
else $a_write = "<Zeroboard ";

// 목록 버튼
if ($is_admin || $member["level"] <= $setup["grant_list"]) $a_list = "<a onfocus=blur() href='{$_SERVER["PHP_SELF"]}?id=$id&page=$page&category=$category&sn=$sn&ss=$ss&sc=$sc&keyword=$keyword&prev_no=$no&sn1=$sn1&divpage=$divpage'>";
...
// 리스트의 상단 부분 출력
$_skinTimeStart = getmicrotime();
include_once $dir . "/list_head.php";
$_skinTime += getmicrotime() - $_skinTimeStart;
...
while ($data = @mysql_fetch_array($result)) {
	...
	if ($data["headnum"] > -2000000000) {
		include_once $dir . "/list_main.php";
	} else {
		include_once $dir . "/list_notice.php";
	}
	$_skinTime += getmicrotime() - $_skinTimeStart;
	$loop_number--;
}
...
// 마무리 부분 출력하는 부분;;
$_skinTimeStart = getmicrotime();
include_once $dir . "/list_foot.php";
$_skinTime += getmicrotime() - $_skinTimeStart;
