...
<script>
	function check_submit() {
		...
		if (!write.table_width.value) {
			alert("게시판 가로크기을 입력하여 주십시요");
			write.table_width.focus();
			return false;
		}
		if (!write.memo_num.value) {
			alert("목록수를 입력하여 주십시요");
			write.memo_num.focus();
			return false;
		}
		if (!write.page_num.value) {
			alert("페이지수를 입력하여 주십시요");
			write.page_num.focus();
			return false;
		}
		return true;
	}
...
<table border=0 cellspacing=1 cellpadding=0 width=100% bgcolor=#b0b0b0>
	<tr height=30>
		<td bgcolor=#3d3d3d colspan=2><img src=images/admin_webboard.gif> </td> </tr> <tr height=1>
	...
	<tr bgcolor=bbbbbb height=30>
		...
		<form method=post action=<?= $_SERVER["PHP_SELF"] ?> name=write onsubmit="return check_submit();">
			...
			<input type=hidden name=exec2 value=<?php if ($no) echo "modify_ok";
												else echo "add_ok"; ?>>
	...
	<tr height=25 bgcolor=#e0e0e0>
		...
		<td>&nbsp;&nbsp; <select name=skinname>
				...
				while ($skin_info = readdir($handle)) {
					if (!eregi("\.", $skin_info)) {
						if ($skin_info == $data["skinname"]) $select = "selected";
						else $select = "";
	...
	<tr height=25 bgcolor=bbbbbb>
		<td colspan=2 align=center style=font-family:Tahoma;font-size:8pt;><b>Edit Properties</b></td>
	</tr>
	...
	<!-- 헤더, 푸터  -->
	<tr height=25 bgcolor=bbbbbb>
		<td colspan=2 align=center style=font-family:Tahoma;font-size:8pt;><b>게시판 상, 하단에 표시될 내용 설정</td>
	</tr>
	...
	<tr height=25 bgcolor=#bbbbbb>
		<td colspan=2 align=center style=font-family:Tahoma;font-size:8pt;><b>추가 기능 설정</b></td>
	</tr>

	<?php unset($check);
	$check[$data["use_alllist"]] = "checked"; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php unset($check);
	$check[$data["use_category"]] = "checked"; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php unset($check);
	if ($data["use_showreply"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_filter"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_status"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_homelink"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_filelink"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_pds"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<tr height=25 bgcolor=#e0e0e0>
		...
		<td>&nbsp;&nbsp;
			<input type=text name=max_upload_size value=2097152 size=10 class=input style=border-color:#b0b0b0> byte &nbsp;&nbsp; (최고한도 : <?php echo get_cfg_var("upload_max_filesize"); ?> byte)
		</td>
	</tr>


	<?php if ($data["use_cart"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_autolink"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_showip"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_comment"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_formmail"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
	...
	<?php if ($data["use_secret"]) $check = "checked";
	else $check = ""; ?>
	<tr height=25 bgcolor=#e0e0e0>
