...
<table border=0 cellspacing=1 cellpadding=0 width=100% bgcolor=#b0b0b0>
	<tr height=30>
		<td bgcolor=#3d3d3d colspan=10><img src=images/admin_webboard.gif> </td> </tr> <tr height=1>
	...
	<script>
			...
			for (i = 0; i < document.write.length; i++) {
				if (document.write[i].type == 'checkbox') {
					if (document.write[i].checked) {
						document.write[i].checked = false;
					} else {
						document.write[i].checked = true;
					}
				}
			}
			return false;
		...
		function sendmail() {
			...
			for (i = 0; i < document.write.length; i++) {
				if (document.write[i].type == 'checkbox') {
					if (document.write[i].checked) {
						cart = cart + "||" + document.write[i].value;
						chked = 1;
					}
				}
			}
	...
	<form method=post action=<?= $_SERVER["PHP_SELF"] ?> name=write>
        ...
        <tr align=center height=23 bgcolor=#e0e0e0>
           ...
           <td style=font-family:Tahoma;font-size:8pt;>";
			if ($data["no"] > 1) echo "<a href={$_SERVER["PHP_SELF"]}?exec=$exec&group_no=$group_no&exec2=del&keyword=$keyword&page=$page&no={$data["no"]}$href onclick=\"return confirm('삭제하시겠습니까?')\">Delete</a>";
			else echo "&nbsp;";
			echo "   </td>
        </tr>
        ";
				...
				<table border=0 cellspacing=0 cellpadding=1>
					<tr>
						...
						<td><img src=images/t.gif height=1><br><select name=movelevel><?php
																						$select[0] = " selected ";
																						for ($i = 1; $i <= 10; $i++)
																							echo "<option value=$i {$select[$i]}>$i Level</option>"; ?></select></td>
						<td><input type=button value='레벨변경' style=border-color:#b0b0b0;background-color:#3d3d3d;color:#ffffff;font-size:8pt;font-family:Tahoma;height:20px; onclick=move_all()>
						</td>
...
<font color=#ffffff style=font-size:8pt;font-family:Tahoma;>
	...
	if ($page > $show_page_num) {
		...
		echo "<a href={$_SERVER["PHP_SELF"]}?exec=$exec&group_no=$group_no&page=$prev_page$href><font color=#ffffff>[Prev]</font></a>";
	}
	...
	if ($total_page > $move_page) {
		...
		echo "<a href={$_SERVER["PHP_SELF"]}?exec=$exec&group_no=$group_no&page=$next_page$href><font color=#ffffff>[Next]</font></a>";
	}
	//페이지 나타내는 부분 끝
