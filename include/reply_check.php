...
if ($no == $reply_data["no"]) $number = "<img src=$dir/arrow.gif border=0 align=absmiddle>";
elseif ($number != "&nbsp;") $number = $roop_number;

// 답글 버튼
if (($is_admin || $member["level"] <= $setup["grant_reply"]) && $reply_data["headnum"] > -2000000000 && $reply_data["headnum"] != -1) $a_reply = "<a href=write.php?$href$sort&no=$reply_data[no]&mode=reply>";
else $a_reply = "<Zeroboard";
// 삭제버튼
if (($is_admin || $member["level"] <= $setup["grant_delete"] || $reply_data["ismember"] == $member["no"] || !$reply_data["ismember"]) && !$reply_data["child"]) $a_delete = "<a href=delete.php?$href$sort&no=$reply_data[no]>";
else $a_delete = "<Zeroboard";
// 수정버튼
if (($is_admin || $member["level"] <= $setup["grant_delete"] || $reply_data["ismember"] == $member["no"] || !$reply_data["ismember"])) $a_modify = "<a href=write.php?$href$sort&no=$reply_data[no]&mode=modify>";
