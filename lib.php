...
if (!eregi("install", $PHP_SELF) && file_exists($_zb_path . "config.php")) {
	...
	if ($autoLoginData["no"]) {
		...
		if (time() - $_SESSION["zb_logged_time"] > $_zbDefaultSetup["login_time"] || $_SESSION["zb_logged_ip"] != $_SERVER["REMOTE_ADDR"]) {
...
// 익스와 넷스케이프일때 처리
if (eregi("msie", $HTTP_USER_AGENT)) $browser = "1";
else $browser = "0";
...
function add_division($board_name = "")
{
	...
	if ($temp[0] >= 5000) {
		...
		if (!$temp[0]) $second_division = 0;
		else $second_division = $temp[0];
...
function member_info()
{
	...
	global $_SESSION, $member_table, $member, $connect;
...
?>
		...
		<body topmargin='0' leftmargin='0' marginwidth='0' marginheight='0' <?= $body ?><?php
			...
			?>
			<table border=0 cellspacing=0 cellpadding=0 width=<?= $width ?> height=1 style="table-layout:fixed;">
				<col width=100%>
				</col>
				<tr>
				...
				<table border=0 cellpadding=0 cellspacing=0 height=20 width=<?= $width ?>>
					<tr>
						<td align=right style=font-family:tahoma,굴림;font-size:8pt;line-height:150%;letter-spacing:0px>
							<font style=font-size:7pt>Copyright 1999-<?= date("Y") ?></font> <a href=http://www.zeroboard.com target=_blank onfocus=blur()>
